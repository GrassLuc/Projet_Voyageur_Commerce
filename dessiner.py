...
class Graph:
    ...
    def calculerCout(self):
        ...
        for i in range(len(self.x)):
            for j in range(i+1, len(self.x)):
                self.c.append(
                    np.sqrt((self.y[j]-self.y[i])**2+(self.x[j]-self.x[i])**2))
        return self.c
    ...
    def afficherEuclidien(self):
        ...
        plt.xlabel('X')
        plt.ylabel('Y')
        ax1.scatter(self.x, self.y, s=self.n*10,
                    c='r', marker='o', label='villes')
    ...
    def afficherTopologique(self):
        ...
        '''
        nx.draw_shell(self.G, nlist=[range(1, self.n+1), range(self.n//2)],
